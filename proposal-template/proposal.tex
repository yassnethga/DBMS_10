...
The FastAPI backend provides CRUD endpoints for all main entities.
Write operations require a valid X-API-Key header.
The profitability calculation is implemented using a JOIN between jobs,
\texttt{job\_materials} and materials together with an aggregation of material costs.
...
\midrule
\texttt{GET} & \texttt{/jobs/profit} & Returns jobs with calculated profit values using JOIN and aggregation. \\
